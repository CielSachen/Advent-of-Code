...
def is_possible(
    ...
    operation: Operation,
    test_value: int,
    should_concatenate: bool = False,  # noqa: FBT001, FBT002
) -> bool:
    ...
    next_remaining_values = remaining_values[1:]

    if should_concatenate:
        return (
            is_possible(
                next_value,
                next_remaining_values,
                Operation.ADDITION,
                test_value,
                should_concatenate,
            )
    ...
    return is_possible(
        next_value, next_remaining_values, Operation.ADDITION, test_value
    ) or is_possible(next_value, next_remaining_values, Operation.MULTIPLICATION, test_value)
