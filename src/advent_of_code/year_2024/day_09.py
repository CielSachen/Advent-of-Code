...
def solve_part_1(puzzle_in: str) -> int:
    ...
    for i, block in enumerate(file_blocks):
        if block == _FREE_SPACE_KEY and i < num_file_blocks + reverse_idx:
            while i < num_file_blocks + reverse_idx:
...
def solve_part_2(puzzle_in: str) -> int:
    ...
    while free_space_idx < num_file_blocks:
        if file_blocks[free_space_idx] == _FREE_SPACE_KEY:
            free_spaces[(num_free_spaces := num_free_spaces + 1)].append(free_space_idx)

            while file_blocks[free_space_idx + 1] == _FREE_SPACE_KEY:
                free_spaces[num_free_spaces].append(free_space_idx := free_space_idx + 1)

        free_space_idx += 1

    reverse_idx = -1

    while reverse_idx >= -num_file_blocks:
        if file_blocks[reverse_idx] != _FREE_SPACE_KEY:
            reverse_indexes = [reverse_idx]

            while (reverse_idx := reverse_idx - 1) >= -num_file_blocks and file_blocks[reverse_idx] == file_blocks[
                reverse_idx + 1
            ]:
            ...
            if front_indexes is not None and num_file_blocks + reverse_indexes[0] > front_indexes[0]:
