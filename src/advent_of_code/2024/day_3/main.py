...
INSTRUCTION_PATTERNS = (r"mul\([0-9]+,[0-9]+\)", r"do\(\)", r"don't\(\)")


def multiply(instruction: str) -> int:
    multiplicand, multiplier = tuple(map(int, instruction[4:-1].split(",", maxsplit=1)))

    return multiplicand * multiplier


def solve_part_one(puzzle_input: str) -> None:
    multiplication_instructions: list[str] = findall(INSTRUCTION_PATTERNS[0], puzzle_input)
...
def solve_part_two(puzzle_input: str) -> None:
    instructions: list[str] = findall("|".join(INSTRUCTION_PATTERNS), puzzle_input)
    is_multiplying = True
