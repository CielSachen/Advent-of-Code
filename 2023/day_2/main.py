...
def parse_game_records(puzzle_input: str) -> list[tuple[int, list[CubeSetSizesByColor]]]:
    ...
    for game_record in puzzle_input.splitlines():
        ...
        game_records.append((int(game_id.replace("Game ", "")), cube_set_sizes))
...
def solve_part_two(puzzle_input: str) -> None:
    ...
    for _, cube_set_sizes in parse_game_records(puzzle_input):
        ...
        for size in cube_set_sizes:
            color_to_minimum_cube_set_size["red"] = max(
                color_to_minimum_cube_set_size["red"], size["red"]
            )
            color_to_minimum_cube_set_size["green"] = max(
                color_to_minimum_cube_set_size["green"], size["green"]
            )
            color_to_minimum_cube_set_size["blue"] = max(
